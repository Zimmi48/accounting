...
- 2026-05-15: `src/Frontend.elm` now exposes a pure `operationSuccessfulRefreshPlan` helper so the mutation-triggered reload seam can be regression-tested without inspecting opaque Lamdera `Cmd`s. The proof lives in `tests/BackendTests.elm`, which loads page 1 plus a load-more page, replays the refresh request after `OperationSuccessful`, and verifies the refreshed list still includes the older month with correct `Y/M/T` header placement and descending April days. Key paths: `src/Frontend.elm`, `tests/BackendTests.elm`.
