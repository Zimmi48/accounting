...
- 2026-05-15T11:24:36Z: Issue #32 revision — fixed transaction check dot visibility. Added `transactionCheckVisualState` and `transactionCheckIndicator` helpers to `src/Frontend.elm` rendering the marker as an explicit elm-ui circle independent of inherited color. Updated `tests/FrontendTests.elm` with regression coverage for marker state transitions, optimistic toggles, and backend refresh seam (preserves checked state for toggled rows). Validation: elm-format, lamdera make src/Frontend.elm, lamdera make src/Backend.elm, npm test (all pass), HTTP 200. Decision merged to `.squad/decisions/decisions.md`. Orchestration log: `.squad/orchestration-log/2026-05-15T11:24:36Z-hicks-issue-32-revision.md`. Commit: 9a67501.
